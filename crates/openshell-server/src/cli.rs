...
async fn run_from_args(mut args: RunArgs, matches: ArgMatches) -> Result<()> {
    ...
    config = config
        ...
        .with_ssh_connect_path(args.ssh_connect_path.clone())
    ...
    if let Some(secret) = args.ssh_handshake_secret.clone() {
...
///
/// The function intentionally does not touch `database_url` or
/// `ssh_handshake_secret` — those secrets are env-only and the loader
/// already rejected them when they appear in the file.
fn merge_file_into_args(args: &mut RunArgs, file: &GatewayFileSection, matches: &ArgMatches) {
    ...
    if let Some(path) = &file.ssh_connect_path
        && arg_defaulted(matches, "ssh_connect_path")
    {
        args.ssh_connect_path.clone_from(path);
    }
    if let Some(skew) = file.ssh_handshake_skew_secs
        && arg_defaulted(matches, "ssh_handshake_skew_secs")
    {
        args.ssh_handshake_skew_secs = skew;
    }
