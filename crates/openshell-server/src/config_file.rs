...
pub struct GatewayFileSection {
    ...
    #[serde(default)]
    pub ssh_connect_path: Option<String>,
    #[serde(default)]
    pub ssh_handshake_skew_secs: Option<u64>,
    #[serde(default)]
    ...
    #[serde(default)]
    pub ssh_handshake_secret: Option<String>,
}
...
pub fn load(path: &Path) -> Result<ConfigFile, ConfigFileError> {
    ...
    if file.openshell.gateway.ssh_handshake_secret.is_some() {
        return Err(ConfigFileError::SecretInFile {
            field: "ssh_handshake_secret",
            env: "OPENSHELL_SSH_HANDSHAKE_SECRET",
            cli: "--ssh-handshake-secret",
        });
    }
...
fn skew_value(n: u64) -> Option<toml::Value> {
    i64::try_from(n).ok().map(toml::Value::Integer)
}
...
"#;
    ...
    #[test]
    fn rejects_ssh_handshake_secret_in_file() {
        let toml = r#"
[openshell.gateway]
ssh_handshake_secret = "leaked"
"#;
        let tmp = write_tmp(toml);
        let err = load(tmp.path()).expect_err("ssh_handshake_secret must be rejected");
        assert!(matches!(
            err,
            ConfigFileError::SecretInFile {
                field: "ssh_handshake_secret",
