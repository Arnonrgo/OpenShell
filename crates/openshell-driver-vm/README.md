...
| `--vm-driver-state-dir DIR` | `OPENSHELL_VM_DRIVER_STATE_DIR` | `target/openshell-vm-driver` | Per-sandbox overlay disks, console logs, image cache, and private `run/compute-driver.sock` UDS. |
...
| `--vm-overlay-disk-mib N` | `OPENSHELL_VM_OVERLAY_DISK_MIB` | `4096` | Sparse writable overlay disk size per sandbox, in MiB. |
